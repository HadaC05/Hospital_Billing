...
class Therapist_Specialties
{
    ...
    function getTypes($params = [])
    {
        ...
        // Get total count
        $countSql = "SELECT COUNT(*) as total FROM user_therapist_specialty WHERE is_active = 1";
    ...
    // function to add new therapist specialty
    function addSpecialty($data)
    {
        ...
        $sql = '
            INSERT INTO user_therapist_specialty (specialty_name, description, is_active)
        ...
        if ($stmt->execute()) {
            echo json_encode(['success' => true, 'message' => 'Therapist specialty added']);
        } else {
            echo json_encode(['success' => false, 'message' => 'Insert failed']);
        }
    }

    // function to update existing therapist specialty
    function updateSpecialty($specialty_name, $description, $specialty_id, $is_active)
    {
        ...
        $sql = "
            UPDATE user_therapist_specialty
...
$therapistSpecialty = new Therapist_Specialties();
...
switch ($operation) {
    case 'getTypes':
        ...
        $therapistSpecialty->getTypes($params);
        break;
    case 'addSpecialty':
        $therapistSpecialty->addSpecialty($data);
        break;
    case 'updateSpecialty':
        $therapistSpecialty->updateSpecialty(
            $data['specialty_name'],
            $data['description'],
            $data['specialty_id'],
            $data['is_active']
    ...
    default:
        ...
        $therapistSpecialty->getTypes(['page' => 1, 'itemsPerPage' => 1000]);
        break;
}
