...
class Labtech_Departments
{
    ...
    function getTypes($params = [])
    {
        ...
        // Get total count
        $countSql = "SELECT COUNT(*) as total FROM user_labtech_department WHERE is_active = 1";
    ...
    // function to add new labtech department
    function addDepartment($data)
    {
        ...
        $sql = '
            INSERT INTO user_labtech_department (department_name, description, is_active)
        ...
        if ($stmt->execute()) {
            echo json_encode(['success' => true, 'message' => 'Lab tech department added']);
        } else {
            echo json_encode(['success' => false, 'message' => 'Insert failed']);
        }
    }

    // function to update existing labtech department
    function updateDepartment($department_name, $description, $department_id, $is_active)
    {
        ...
        $sql = "
            UPDATE user_labtech_department
...
$labtechDepartment = new Labtech_Departments();
...
switch ($operation) {
    case 'getTypes':
        ...
        $labtechDepartment->getTypes($params);
        break;
    case 'addDepartment':
        $labtechDepartment->addDepartment($data);
        break;
    case 'updateDepartment':
        $labtechDepartment->updateDepartment(
    ...
    default:
        ...
        $labtechDepartment->getTypes(['page' => 1, 'itemsPerPage' => 1000]);
        break;
}
