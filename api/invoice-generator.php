...
class Invoices
{
    ...
    function getBillableItems($admission_id)
    {
        ...
        try {
            ...
            // Get all administered medicines including billed ones
            $sqlMed = "
                SELECT 
                    ...
                    COALESCE((
                        ...
                        WHERE reference_table = 'request_medicine_items' 
                        AND reference_id = rmi.item_id
                        LIMIT 1
                    ...
                    CASE 
                        WHEN rmi.billed_status = 'yes' THEN 'Paid' 
                        ELSE 'Unpaid' 
            ...
            // Get all lab tests including billed ones
            $sqlLab = "
                SELECT 
                    ...
                    COALESCE((
                        SELECT coverage_amount 
                        FROM bill_invoice_items 
                        WHERE reference_table = 'request_labtest_items' 
                        AND reference_id = rli.item_id
                        LIMIT 1
                    ), 0) AS coverage_amount,
                    'Lab Test' AS service_type_name, 
                    3 AS svc_type_id,
                    'request_labtest_items' AS reference_table,
                    CASE WHEN rli.billed_status = 'yes' THEN 'Paid' ELSE 'Unpaid' END as payment_status
