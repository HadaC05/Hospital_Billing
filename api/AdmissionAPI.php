...
class AdmissionAPI
{
    ...
    /**
     * Assign or transfer a room for an admission by creating a new row in tbl_room_stay.
     * - Ensures a tbl_room_assignment header exists for the admission
     * - Closes any active stay (end_date = startDate)
     * - Inserts a new stay row with start_date and assigned_by
     * - Updates room availability flags
     */
    public function assignRoom($admissionId, $roomId, $assignedBy, $startDate, $transferReason = null)
    {
        header('Content-Type: application/json');
        try {
            if ($admissionId <= 0 || $roomId <= 0 || $assignedBy <= 0) {
            ...
            // Get or create room_assignment header for this admission
            $stmt = $pdo->prepare('SELECT room_assignment_id FROM tbl_room_assignment WHERE admission_id = ? FOR UPDATE');
            $stmt->execute([(int)$admissionId]);
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($row) {
                $assignmentId = (int)$row['room_assignment_id'];
            } else {
                $stmt = $pdo->prepare('INSERT INTO tbl_room_assignment (admission_id, record_date) VALUES (?, ?)');
                $stmt->execute([(int)$admissionId, $startDate]);
                $assignmentId = (int)$pdo->lastInsertId();
            }

            // Close any active stay and free previous room
            $stmt = $pdo->prepare("SELECT room_stay_id, room_id FROM tbl_room_stay WHERE room_assignment_id = ? AND end_date = '0000-00-00' FOR UPDATE");
            $stmt->execute([$assignmentId]);
            $activeStay = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($activeStay) {
                // Set end date to startDate of new stay
                $stmt = $pdo->prepare('UPDATE tbl_room_stay SET end_date = ? WHERE room_stay_id = ?');
                $stmt->execute([$startDate, (int)$activeStay['room_stay_id']]);

                // Free previous room
                $stmt = $pdo->prepare('UPDATE tbl_room SET is_available = 1 WHERE room_id = ?');
                $stmt->execute([(int)$activeStay['room_id']]);
            }

            // Assign new room stay row
            $sql = "INSERT INTO tbl_room_stay (room_assignment_id, room_id, start_date, assigned_by, transfer_reason) VALUES (?, ?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$assignmentId, $roomId, $startDate, $assignedBy, $transferReason]);

            // Mark new room as unavailable
            $stmt = $pdo->prepare('UPDATE tbl_room SET is_available = 0 WHERE room_id = ?');
            $stmt->execute([(int)$roomId]);

            $pdo->commit();
            echo json_encode(['status' => 'success', 'message' => 'Room assigned successfully', 'room_assignment_id' => $assignmentId]);
        } catch (Exception $ex) {
            if (isset($pdo) && $pdo instanceof PDO && $pdo->inTransaction()) {
                $pdo->rollBack();
            }
            echo json_encode(['status' => 'error', 'message' => 'Failed to assign room: ' . $ex->getMessage()]);
        }
    }
    ...
    function getAdmission($admissionId)
    {
        ...
        try {
            ...
            // Get full room stay history for this admission
            $sql = "SELECT 
                        rs.room_stay_id,
                        rs.room_assignment_id,
                        rs.room_id,
                        rs.start_date,
                        CASE WHEN rs.end_date = '0000-00-00' THEN NULL ELSE rs.end_date END AS end_date,
                        rs.transfer_reason,
                        r.room_number,
                        rt.room_type_name,
                        r.daily_rate,
                    ...
                    JOIN tbl_room r ON rs.room_id = r.room_id
                    JOIN tbl_room_type rt ON r.room_type_id = rt.room_type_id
                    LEFT JOIN users u ON rs.assigned_by = u.user_id
                    WHERE ra.admission_id = ?
                    ORDER BY rs.start_date ASC, rs.room_stay_id ASC";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$admissionId]);
            $roomStays = $stmt->fetchAll(PDO::FETCH_ASSOC);

            // Determine ER initial charge applicability (mirrors logic in invoice.php)
            $erStmt = $pdo->prepare(
                "SELECT 1
                 FROM tbl_room_stay rs
                 JOIN tbl_room r ON rs.room_id = r.room_id
            ...
            $erStmt->execute([$admissionId]);
            $erInitialApplied = $erStmt->fetch(PDO::FETCH_ASSOC) ? true : false;
            ...
            $response = [
                ...
                // Provide both snake_case and camelCase for compatibility with various frontends
                'room_stays' => $roomStays,
                'roomStays' => $roomStays,
                'er_initial_charge_applied' => $erInitialApplied,
                'services' => $services,
...
switch ($operation) {
    ...
    case "assignRoom":
        ...
        $transfer_reason = isset($data['transfer_reason']) ? $data['transfer_reason'] : null;
        $obj->assignRoom($admissionId, $roomId, $assignedBy, $startDate, $transfer_reason);
        break;
