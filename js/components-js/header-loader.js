...
// Minimal global SweetAlert fallback to avoid ReferenceError when SweetAlert2 isn't loaded
if (!window.Swal) {
    window.Swal = {
        fire: (opts) => {
            try {
                const o = typeof opts === 'string' ? { title: 'Notice', text: opts } : (opts || {});
                const title = o.title || 'Notice';
                const text = o.text || '';
                const icon = o.icon ? `[${String(o.icon).toUpperCase()}] ` : '';
