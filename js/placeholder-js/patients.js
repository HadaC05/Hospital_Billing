...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // ----- Utilities to sanitize values for UI rendering -----
    function safeText(value, fallback = '-') {
        const v = value == null ? '' : String(value).trim();
        if (!v) return fallback;
        const low = v.toLowerCase();
        if (low === 'n/a' || low === 'na' || low === 'undefined' || low === 'null') return fallback;
        return v;
    }
    ...
    async function loadLabTestsForModal() {
        try {
            const response = await axios.get(`${baseApiUrl}/masterfiles-php/get-labtests.php`, {
                params: {
    ...
    async function loadSurgeriesForModal() {
        try {
            const response = await axios.get(`${baseApiUrl}/masterfiles-php/get-surgeries.php`, {
                params: {
    ...
    async function loadTreatmentsForModal() {
        try {
            const response = await axios.get(`${baseApiUrl}/masterfiles-php/get-treatments.php`, {
                params: {
    ...
    // ========= Transfer Room Modal Logic =========
    const transferRoomModalEl = document.getElementById('transferRoomModal');
    const newRoomSelect = document.getElementById('new-room-id');
    ...
    async function loadRoomsForTransfer() {
        try {
            const response = await axios.get(`${baseApiUrl}/masterfiles-php/get-rooms.php`, {
                params: {
                    operation: 'getRooms',
                    json: JSON.stringify({}),
                    page: 1,
            ...
            const data = response.data;
            if (data.success) {
                roomsCache = (data.rooms || []).filter(r => Number(r.is_available) === 1);
                if (newRoomSelect) {
                    newRoomSelect.innerHTML = '<option value="">Select room</option>';
                    roomsCache.forEach(room => {
                        const opt = document.createElement('option');
                        opt.value = room.room_id;
                        opt.textContent = `${room.room_number} (${room.room_type_name})`;
                        newRoomSelect.appendChild(opt);
                    });
                }
            } else {
                Swal.fire({ title: 'Error', text: data.message || 'Failed to load rooms', icon: 'error' });
            }
        } catch (err) {
            ...
            try {
                ...
                const payload = {
                    ...
                    json: JSON.stringify({
                        ...
                        // Reason not stored in DB yet; included for future use
                        transfer_reason: transferReason,
                        assigned_by: Number(user.user_id)
                    })
                };
                ...
                if (data && data.status === 'success') {
                    ...
                    Swal.fire({ title: 'Error', text: (data && data.message) || 'Failed to transfer room', icon: 'error' });
                }
            } catch (err) {
                console.error('Transfer room error:', err);
                Swal.fire({ title: 'Error', text: 'Failed to transfer room', icon: 'error' });
            }
        });
    }
    ...
    if (transferRoomModalEl) {
        transferRoomModalEl.addEventListener('show.bs.modal', async () => {
            if (transferDateInput) transferDateInput.valueAsDate = new Date();
    ...
    async function loadMedicinesForDispense() {
        try {
            ...
            const response = await axios.get(`${baseApiUrl}/masterfiles-php/get-medicines.php`, {
                params: {
    ...
    if (labTestsSubmitBtn) {
        labTestsSubmitBtn.addEventListener('click', async () => {
            try {
                ...
                const dateInput = labTestDateInput ? labTestDateInput.value : '';
                const items = labTestsCart.map(i => ({ labtest_id: Number(i.labtest_id), quantity: Number(i.quantity) }));
                const isDoctor = (user.role_name || '').toLowerCase().includes('doctor');
                // If doctor, submit as a request; otherwise, submit as performed (existing behavior)
                const payload = isDoctor
                    ? {
                        operation: 'requestLabTestsBatch',
                        json: JSON.stringify({
                            admission_id: Number(currentAdmissionId),
                            items,
        ...
        try {
            ...
            if (data.success && Array.isArray(data.patients)) {
                ...
                patients.forEach(patient => {
                    const lname = safeText(patient.patient_lname);
                    const fname = safeText(patient.patient_fname);
                    const mname = safeText(patient.patient_mname, '');
                    const fullName = `${lname}, ${fname}${mname ? ' ' + mname.charAt(0) + '.' : ''}`;
                    const currentRoom = patient.current_room_number ? safeText(patient.current_room_number) : '-';
                    const row = `
                        <tr>
                            <td>${fullName}</td>
                            <td>${safeText(patient.mobile_number)}</td>
                            <td>${currentRoom}</td>
                            <td>
        ...
        try {
            ...
            if (data.success) {
                ...
                patientInfoElement.innerHTML = `
                    <div class="col-md-6">
                        <h4>${safeText(patient.patient_fname)} ${safeText(patient.patient_mname, '') ? safeText(patient.patient_mname) + ' ' : ''}${safeText(patient.patient_lname)} ${safeText(patient.patient_suffix, '')}</h4>
                        <p><strong>Birth Date:</strong> ${formatDate(patient.birthdate)}</p>
                        <p><strong>Age:</strong> ${age} years old</p>
                        <p><strong>Gender:</strong> ${safeText(patient.gender)}</p>
                        <p><strong>Marital Status:</strong> ${safeText(patient.marital_status)}</p>
                        <p><strong>Address:</strong> ${safeText(patient.address)}</p>
                        <p><strong>Contact:</strong> ${safeText(patient.mobile_number)}</p>
                        <p><strong>Email:</strong> ${safeText(patient.email)}</p>
                    </div>
                    <div class="col-md-6">
                        <h5>Emergency Contact</h5>
                        <p><strong>Name:</strong> ${safeText(patient.em_contact_name)}</p>
                        <p><strong>Relationship:</strong> ${safeText(patient.em_contact_relationship)}</p>
                        <p><strong>Contact:</strong> ${safeText(patient.em_contact_number)}</p>
                        <p><strong>Email:</strong> ${safeText(patient.em_contact_email)}</p>
                        <p><strong>Address:</strong> ${safeText(patient.em_contact_address)}</p>
                        <h5 class="mt-3">Parent/Guardian Information</h5>
                        <p><strong>Name:</strong> ${safeText(patient.parent_name, 'N/A')}</p>
                        <p><strong>Contact:</strong> ${safeText(patient.parent_contact, 'N/A')}</p>
                    </div>
                `;
                ...
                if (admissions.length === 0) {
                    admissionListElement.innerHTML = '<tr><td colspan="7" class="text-center">No admissions found.</td></tr>';
                } else {
                    admissionListElement.innerHTML = '';
                    admissions.forEach(admission => {
                        const dischargeRaw = (admission.discharge_date && admission.discharge_date !== '0000-00-00') ? admission.discharge_date : '';
                        const admissionDate = formatDate(admission.admission_date);
                        const dischargeDate = dischargeRaw ? formatDate(dischargeRaw) : 'Not discharged';
                        const row = `
                            <tr>
                                <td>${admission.admission_id}</td>
                                <td>${admissionDate}</td>
                                <td>${dischargeDate}</td>
                                <td>${safeText(admission.room_number, 'Not assigned')}</td>
                                <td>${safeText(admission.admission_reason)}</td>
                                <td>${safeText(admission.status)}</td>
                                <td>
                ...
                if (insurance.length === 0) {
                    ...
                    insurance.forEach(policy => {
                        ...
                        const row = `
                            <tr>
                                <td>${safeText(policy.policy_number)}</td>
                                <td>${safeText(policy.provider_name)}</td>
                                <td>${startDate}</td>
                                <td>${endDate}</td>
                                <td>${safeText(policy.status)}</td>
                            </tr>
                        `;
    ...
    async function loadAdmissionDetails(admissionId) {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                // Determine ER Initial Charge status from backend boolean, fallback to old admission field if any
                const erApplied = (data.er_initial_charge_applied === true)
                    || (String(data.er_initial_charge_applied) === '1')
                    || (String(admission.er_initial_charge) === '1');

                admissionInfoElement.innerHTML = `
                    <div class="col-md-6">
                        <h4>Admission #${admission.admission_id}</h4>
                        <p><strong>Patient:</strong> ${safeText(admission.patient_fname)} ${safeText(admission.patient_mname, '') ? safeText(admission.patient_mname) + ' ' : ''}${safeText(admission.patient_lname)}</p>
                        <p><strong>Doctor:</strong> ${safeText(admission.doctor_name, 'Not assigned')}</p>
                        <p><strong>Admission Date:</strong> ${admissionDate}</p>
                        <p><strong>Reason:</strong> ${safeText(admission.admission_reason)}</p>
                    </div>
                    <div class="col-md-6">
                        <p><strong>Discharge Date:</strong> ${dischargeDate}</p>
                        <p><strong>ER Initial Charge:</strong> ${erApplied ? 'Applied' : 'Not Applied'}</p>
                    </div>
                `;

                // Display current active room stay prominently
                const currentRoom = data.current_room || null;
                let currentRoomHtml = '';
                if (currentRoom) {
                    const roomNo = safeText(currentRoom.room_number, 'N/A');
                    const roomType = safeText(currentRoom.room_type_name, '');
                    const sinceDate = currentRoom.start_date ? formatDate(currentRoom.start_date) : '';
                    const rate = (currentRoom.daily_rate !== undefined && currentRoom.daily_rate !== null)
                        ? Number(currentRoom.daily_rate).toFixed(2)
                        : '';
                    currentRoomHtml = `
                        <div class="col-12 mt-2">
                            <div class="alert alert-info d-flex align-items-start" role="alert">
                                <i class="fas fa-bed me-2 mt-1"></i>
                                <div>
                                    <div><strong>Current Room:</strong> ${roomNo}${roomType ? ` (${roomType})` : ''}</div>
                                    <div class="small text-muted">${sinceDate ? `Since: ${sinceDate}` : ''}${rate ? ` • Daily rate: ${rate}` : ''}</div>
                                </div>
                            </div>
                        </div>`;
                } else {
                    currentRoomHtml = `
                        <div class="col-12 mt-2">
                            <div class="alert alert-secondary d-flex align-items-start" role="alert">
                                <i class="fas fa-bed me-2 mt-1"></i>
                                <div><strong>Current Room:</strong> No current room assigned</div>
                            </div>
                        </div>`;
                }
                admissionInfoElement.innerHTML += currentRoomHtml;

                // Display room stays
                if (roomStayListElement) {
                    if (roomStays.length === 0) {
                        roomStayListElement.innerHTML = '<tr><td colspan="4" class="text-center">No room stays found.</td></tr>';
                    } else {
                        roomStayListElement.innerHTML = '';
                        roomStays.forEach(roomStay => {
                            const startDate = formatDate(roomStay.start_date);
                            const reasonTransfer = roomStay.transfer_reason ? safeText(roomStay.transfer_reason) : '-';
                            const row = `
                                <tr>
                                    <td>${safeText(roomStay.room_number, 'Not assigned')}</td>
                                    <td>${startDate}</td>
                                    <td>${reasonTransfer}</td>
                                    <td>${safeText(roomStay.status, 'Active')}</td>
                                </tr>
                            `;
                            roomStayListElement.innerHTML += row;
                        });
                    }
                }
                ...
                if (medications.length === 0) {
                    ...
                    medications.forEach(item => {
                        const dateGiven = item.date_given ? formatDate(item.date_given, '') : '';
                        const row = `
                            <tr>
                                <td>${safeText(item.med_name, '')}</td>
                                <td class="text-end">${item.quantity}</td>
                                <td>${safeText(item.administered_by_name, '')}</td>
                                <td>${dateGiven}</td>
                            </tr>
                        `;
                ...
                if (labtests.length === 0) {
                    ...
                    labtests.forEach(item => {
                        ...
                        const prettyDate = datePerf ? formatDate(datePerf, '') : '';
                        const row = `
                            <tr>
                                <td>${safeText(item.test_name, '')}</td>
                                <td>${item.quantity || 0}</td>
                                <td>${safeText(item.performed_by_name, '')}</td>
                                <td>${prettyDate}</td>
                            </tr>
                        `;
                ...
                if (surgeries.length === 0) {
                    ...
                    surgeries.forEach(surgery => {
                        ...
                        const prettyDate = recordDate ? formatDate(recordDate, '') : '';
                        const row = `
                            <tr>
                                <td>${safeText(surgery.surgery_name, '')}</td>
                                <td>${surgery.quantity || 0}</td>
                                <td>${safeText(surgery.performed_by_name, '')}</td>
                                <td>${prettyDate}</td>
                            </tr>
                        `;
                ...
                if (treatments.length === 0) {
                    ...
                    treatments.forEach(treatment => {
                        ...
                        const row = `
                            <tr>
                                <td>${safeText(treatment.treatment_name, '')}</td>
                                <td>${treatment.quantity || 0}</td>
                                <td>${safeText(treatment.performed_by_name, '')}</td>
                                <td>${prettyDate}</td>
                            </tr>
                        `;
                ...
                if (String(admission.agreement_signed) === '1') {
                    consentCheckbox.checked = true;
                    consentSignedBy.textContent = safeText(admission.agreement_signed_by, 'Not available');
                    consentDate.textContent = admission.agreement_date ? formatDate(admission.agreement_date) : 'Not available';
                } else {
