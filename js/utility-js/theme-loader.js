/**
 * Hospital Billing System Theme Loader (Utility)
 * - Ensures Font Awesome and base theme CSS are present
 * - Applies UI enhancements (icons, tables, cards, buttons)
 * - Adds theme initialization and toggle (light/dark) with Bootstrap 5.3 data-bs-theme
 */

'use strict';

// Idempotent guard to prevent double-execution if included multiple times
if (!window.__HB_THEME_UTILITY_LOADED) {
    window.__HB_THEME_UTILITY_LOADED = true;

    // Minimal theme init and toggle available globally
    (function initTheme() {
        try {
    ...
    function setTheme(theme) {
        try {
            ...
            document.documentElement.dataset.bsTheme = t; // Bootstrap 5.3 compatible
