...
document.addEventListener('DOMContentLoaded', () => {
  ...
  checkMedicineDispensingPermission();
});

// Check if user has medicine dispensing permission
async function checkMedicineDispensingPermission() {
  try {
    const user = JSON.parse(localStorage.getItem('user'));
    if (!user) return;

    const response = await axios.post('../api/get-permissions.php', {
    ...
    if (data.status === 'success') {
      ...
      const hasPermission = permissions.some(p => p.name === 'medicine_dispensing');
      
      const medicineDispensingBtn = document.getElementById('medicine-dispensing-btn');
      if (medicineDispensingBtn) {
        if (!hasPermission) {
          medicineDispensingBtn.disabled = true;
          medicineDispensingBtn.title = 'Medicine dispensing permission required';
          medicineDispensingBtn.innerHTML = '<i class="fas fa-lock me-2"></i>Medicine Dispensing (No Permission)';
        }
      }
    }
  } catch (error) {
    console.error('Error checking medicine dispensing permission:', error);
  }
}

// Check permission and navigate function
async function checkPermissionAndNavigate(permissionName, url) {
  try {
    const user = JSON.parse(localStorage.getItem('user'));
    if (!user) {
    ...
    if (data.status === 'success') {
      ...
      Swal.fire({
        title: 'Error',
        text: 'Unable to verify permissions.',
        icon: 'error',
        confirmButtonText: 'OK'
      });
    }
...
function loadPharmacistDashboard() {
  axios.post('../api/get-admissions.php', { operation: 'getAdmissions', page: 1, itemsPerPage: 500, search: '' })
    .then(resp => {
      ...
      renderPharmRecent(rows);
    })
    .catch(err => { console.error('Pharmacist dashboard load error', err); renderPharmKpis([]); renderPharmRecent([]); });
}
...
function renderPharmRecent(rows) {
  const tbody = document.getElementById('pharm-recent'); if (!tbody) return;
  tbody.innerHTML='';
